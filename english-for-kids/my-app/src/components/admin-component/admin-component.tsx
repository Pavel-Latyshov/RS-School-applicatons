...
const AdminComponent = ({ dataJson, user }: any) => {
  ...
  for (const key of Object.entries(dataJson.sets)) {
    ...
    for (const entry of Object.entries(wordsArr)) {
      // console.log(entry[0]);
      const wordsLength: any = entry[1];
      const wordsLengthArr = [];
      for (const w of Object.entries(wordsLength)) {
        const word: any = w[1];
  ...
  const renderAdminComponent = arr.map((i) => (
    <AdminSingleComponent category={i[0]} words={i[1]} img={i[2]} user={user} />
  ));

...
const GameComponent = ({ item, vataJson, wordCheck, setWordCheck, setStars, stars, starsClear, hideNav }: any) => {
    ...
    const gameRender = imageArr.map((item: any) => {
        if (vataJson === true) {
            ...
            return (
                <Game shuffleArr={shuffleArr} setWordCheck={setWordCheck} wordCheck={wordCheck} item={item} setStars={setStars} checkLength={checkLength}/>
            )
        }
    })
    return (
        <div onClick={hideNav}>
            ...
            <div  className={css.main_cards__wrapper}>
                {winArr.length === checkLength ? <ResultComponent checkLength={checkLength} starsClear={starsClear}/> : gameRender}</div>
        </div>
    )
}
